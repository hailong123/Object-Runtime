...
/// Represents an instance of a class. 代表一个类的实例
//向 object 发送消息时, Runtime库会根据object的isa指针, 找到这个实例的所属类, 然后在类的方法缓存列表里面进行方法付查找,如果找到了就进行调用返回, 如果没有找到就到类的方法列表里面进行查找, 查找到后进行调用返回并缓存到方法列表中,若还是没有找到, 就通过类的super指针,从父类的方法缓存列表里面进行查找, 查找到后进行调用返回并缓存到父类的方法列表中,依次类推直到查找到元类对象, 若还是没有找到方法 则进入方法的消息转发阶段...
struct objc_object {
