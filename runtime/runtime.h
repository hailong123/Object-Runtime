...
struct objc_class {
    //isa 指针指向 Meta Class(元类对象) 因为一个Objc的类的本身也是一个Object, 为了让处理这个关系, runtime就创建了Meta class, 当给类发送[NSObject alloc] 这样的消息时, 实际上是把这个消息给了 Class Object
    Class _Nonnull isa  OBJC_ISA_AVAILABILITY;

#if !__OBJC2__ //这是老版本 runtime使用的结构体, 新的版本已对此进行优化 使用 rw_t, ro_t进行存储
    ...
    struct objc_cache * _Nonnull cache                       OBJC2_UNAVAILABLE;//方法缓存, 对象接到一个消息会根据isa指针查找消息对象,这时会在method中遍历, 如果命中, 常用的方法调用时就能够提高调用的效率
...
//objc_method_list 结构体存储着objc_method的数组列表
...
#endif
struct objc_cache {
    //指定分配缓存bucket的总数, runtime使用这个字段确定性查找数组的索引位置
    unsigned int mask /* total = mask + 1 */                 OBJC2_UNAVAILABLE;
    //实际占用缓存bucket的总数
    unsigned int occupied                                    OBJC2_UNAVAILABLE;
    //指向method数据结构指针的数组, 这个数组总数不能超过 mask+1 但是指针是可能为空的, 这就是表示缓存bucket没有被占用,数组会随着时间增长
